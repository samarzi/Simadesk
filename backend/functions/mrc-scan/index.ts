 ...
 * ошибки API — в repricer_errors (см. backend/migrations).
 *
 * Запускается по расписанию через pg_cron (см. backend/migrations).
 * Требует авторизации (Authorization: Bearer <anon|service_role key>) — pg_cron
 * передаёт ключ через pg_net.http_post.
 * Deploy: docker compose restart functions
 */
...
async function logEvent(db: any, ev: Record<string, any>): Promise<void> {
  try {
    await db.from('repricer_events').insert(ev);
  } catch {
    // лог не должен валить основной сканер
  }
}

async function logError(db: any, err: Record<string, any>): Promise<void> {
  try {
    await db.from('repricer_errors').insert(err);
  } catch {
    // лог не должен валить основной сканер
  }
}
...
async function persistRules(db: any, ruleIds: Set<string>, rulesById: Map<string, any>): Promise<void> {
  for (const id of ruleIds) {
    const rule = rulesById.get(id);
    if (rule) await db.from('repricer_rules').update({ data: rule.data }).eq('id', rule.id);
  }
}
...
async function fetchWbBuyerPrices(nmIds: number[], db: any): Promise<Map<number, number>> {
...
async function processWb(db: any, rules: any[], log: any[]): Promise<void> {
  const byStore = collectItemsByStore(rules, 'wb');
  if (byStore.size === 0) return;

  for (const [storeId, entries] of byStore) {
    const { data: store } = await db.from('wb_stores').select('api_key, name').eq('id', storeId).single();
    ...
    const priceMap = await fetchWbGoods(store.api_key, nmIds, db);
    const showcaseMap = await fetchWbBuyerPrices(nmIds, db);
    ...
    if (updates.length > 0) {
      const ok = await uploadWbPrices(store.api_key, updates, store, db, log);
    ...
    await persistRules(db, touchedRuleIds, rulesById);
  }
}
...
async function fetchOzonBuyerPrices(storeId: string, offerIds: string[], db: any): Promise<Map<string, number>> {
...
async function processOzon(db: any, rules: any[], log: any[]): Promise<void> {
  const byStore = collectItemsByStore(rules, 'ozon');
  if (byStore.size === 0) return;

  for (const [storeId, entries] of byStore) {
    const { data: store } = await db.from('ozon_stores').select('client_id, api_key, name').eq('id', storeId).single();
    ...
    const priceMap = await fetchOzonPrices(headers, offerIds, store, db);
    const showcaseMap = await fetchOzonBuyerPrices(storeId, offerIds, db);
...
async function fetchYandexBuyerPrices(storeId: string, offerIds: string[], db: any): Promise<Map<string, number>> {
  const showcaseMap = new Map<string, number>();
  if (offerIds.length === 0) return showcaseMap;
...
async function processYandex(db: any, rules: any[], log: any[]): Promise<void> {
  const byStore = collectItemsByStore(rules, 'yandex');
  if (byStore.size === 0) return;

  for (const [storeId, entries] of byStore) {
    const { data: store } = await db.from('yandex_stores').select('api_key, campaign_id, business_id, name').eq('id', storeId).single();
    ...
    const priceMap = await fetchYandexPrices(headers, store.campaign_id, store, db, offerIds);
    const showcaseMap = await fetchYandexBuyerPrices(storeId, offerIds, db);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { data: rules, error } = await db.from('repricer_rules').select('id, data');
    ...
    await processOzon(db, mrcRules, log);
    await sleep(300);
    await processWb(db, mrcRules, log);
    await sleep(300);
    await processYandex(db, mrcRules, log);
