-- Final get_storefront: logo_url via JOIN companies, phone, Yandex offer_id fallback,
-- dimensions, category, groups, no LIMIT, no description
...
BEGIN
  ...
  wb AS (
    SELECT DISTINCT ON (CASE WHEN vendor_code = '' THEN source_id ELSE vendor_code END)
      ...
      sort_order, is_hidden, ozon_sku, weight_kg, length_cm, width_cm, height_cm
    FROM wb_visible
    ORDER BY
