-- Add buyer-facing price columns for all marketplaces
ALTER TABLE wb_products      ADD COLUMN IF NOT EXISTS buyer_price     numeric;
ALTER TABLE yandex_products  ADD COLUMN IF NOT EXISTS offer_price     numeric;
ALTER TABLE ozon_products    ADD COLUMN IF NOT EXISTS marketing_price numeric;

-- Update get_storefront to use buyer-facing prices
...
BEGIN
  ...
  WITH wb_visible AS (
    SELECT
      ...
      -- buyer_price = WB SPP price synced from public API; fallback to seller-discounted price
      COALESCE(p.buyer_price,
        ROUND(p.price * (1 - COALESCE(p.discount, 0)::numeric / 100))
      )::numeric                                                                AS price,
      p.price                                                                   AS original_price,
      CASE
        WHEN p.buyer_price IS NOT NULL AND p.price > 0
          THEN ROUND((1 - p.buyer_price / NULLIF(p.price, 0)) * 100)
  ...
  oz_visible AS (
    SELECT
      ...
      p.name                                                                    AS title,
      p.images,
      -- marketing_price = витринная цена Ozon с учётом акций самого Ozon; fallback → price (цена продавца)
      COALESCE(NULLIF(p.marketing_price, 0), p.price)::numeric                 AS price,
      COALESCE(NULLIF(p.old_price, 0), p.price)::numeric                       AS original_price,
      CASE
        WHEN COALESCE(NULLIF(p.marketing_price, 0), p.price) < COALESCE(NULLIF(p.old_price, 0), p.price)
          THEN ROUND((1 - COALESCE(NULLIF(p.marketing_price, 0), p.price)
               / NULLIF(COALESCE(NULLIF(p.old_price, 0), p.price), 0)) * 100)
        ELSE 0
      END                                                                       AS discount,
  ...
  ya_visible AS (
    SELECT
      ...
      p.name                                                                    AS title,
      p.pictures                                                                AS images,
      -- offer_price = витринная цена на Яндекс.Маркете (из /offer-prices); fallback basic_price
      COALESCE(p.offer_price, p.basic_price)::numeric                          AS price,
      p.basic_price::numeric                                                    AS original_price,
      CASE
        WHEN p.offer_price IS NOT NULL AND p.basic_price > 0 AND p.offer_price < p.basic_price
          THEN ROUND((1 - p.offer_price / NULLIF(p.basic_price, 0)) * 100)
        ELSE 0
      END                                                                       AS discount,
