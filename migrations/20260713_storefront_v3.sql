-- Add optional admin title to banners (internal label only, not shown on storefront)
ALTER TABLE storefront_banners ADD COLUMN IF NOT EXISTS title text DEFAULT NULL;

-- Recreate get_storefront: deduplicate by (vendor_code, source) → cheapest price per MP
-- Rule: 1 vendor_code per marketplace = 1 entry (cheapest among all stores in that MP)
