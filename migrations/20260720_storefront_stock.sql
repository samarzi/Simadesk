-- Add stock-related settings to storefront_settings
...
-- Recreate get_storefront: adds stock per product + stock-based filtering
...
BEGIN
  SELECT ss.company_id,
         ...
         COALESCE(ss.hide_out_of_stock, false)
  INTO v_company_id, v_settings, v_hide_oos
  ...
  WITH wb_visible AS (
    ...
    WHERE s.company_id = v_company_id
      ...
      AND (NOT v_hide_oos OR COALESCE(p.stock_total, 0) > 0)
  ),
  wb AS (
    SELECT DISTINCT ON (CASE WHEN vendor_code = '' THEN source_id ELSE vendor_code END)
      ...
      sort_order, is_hidden, ozon_sku, weight_kg, length_cm, width_cm, height_cm, stock
    FROM wb_visible
    ORDER BY
  ...
  oz_visible AS (
    ...
    WHERE s.company_id = v_company_id
      ...
      AND (NOT v_hide_oos OR (COALESCE(p.stock_fbs, 0) + COALESCE(p.stock_fbo, 0)) > 0)
  ),
  oz AS (
    SELECT DISTINCT ON (CASE WHEN vendor_code = '' THEN source_id ELSE vendor_code END)
      source, source_id, market_model_id, title, images, price, original_price,
  ...
  ya_visible AS (
    ...
    WHERE s.company_id = v_company_id
      ...
      AND NOT COALESCE(ov.is_hidden, false)
      AND p.name IS NOT NULL AND p.name != ''
      AND (NOT v_hide_oos OR COALESCE(p.stock_available, p.stock_total, 0) > 0)
  ),
  ya AS (
    SELECT DISTINCT ON (CASE WHEN vendor_code = '' THEN source_id ELSE vendor_code END)
      source, source_id, market_model_id, title, images, price, original_price,
