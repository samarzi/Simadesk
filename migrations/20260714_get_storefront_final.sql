-- Final get_storefront:
--   • ozon_sku field for correct Ozon product URLs (uses sku, not product_id)
--   • Group-based LIMIT 300: picks 300 unique vendor_code groups, returns ALL
--     marketplace variants for each group (so cross-MP grouping is never split)
...
BEGIN
  ...
  -- Group-based limit: 300 unique vendor_code groups; all MP variants per group included
  top_groups AS (
    SELECT DISTINCT
