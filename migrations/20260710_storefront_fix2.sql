-- Fix get_storefront:
--   1. Add ozon_sku field so client can build correct Ozon URLs (uses sku, not product_id)
--   2. Group-based LIMIT: limit 300 unique vendor_code groups (not individual rows)
--      so cross-marketplace products are always included together
...
BEGIN
  ...
  -- ── Per-marketplace deduplication: cheapest price per (marketplace, vendor_code) ──────────
  ...
  -- ── Group-based limit: top 300 unique vendor_code groups (alphabetically) ──────────────
  -- This ensures that if vendor_code "ABC" exists in WB and Ozon, BOTH are included.
  top_groups AS (
    SELECT DISTINCT
