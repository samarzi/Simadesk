...
export class RepricerModule {
  ...
  deleteAllDeletedCosts(): void {
    ...
    this.render();
    try { window.app?.toast?.(`🗑 Удалено ${toDelete.length} записей`, 'success'); } catch (e) { debug.warn('[RepricerModule] swallowed error', e); }
  }
  ...
  exportCostsTemplate(): void {
    ...
    try { window.app?.toast?.(`📥 Скачан шаблон с ${products.length} товарами`, 'success'); } catch (e) { debug.warn('[RepricerModule] swallowed error', e); }
  }
