...
export function renderPnL(k: KPI, onFilterClick = 'window.analyticsModule?.openOrdersFiltered'): string {
  ...
  const rows: Row[] = [
    { icon: '🟢', label: 'Выручка (нетто)',  value: k.revenue,        tint: 'var(--green)',    kind: 'income',  filter: 'delivered' },
    { icon: '↩',  label: 'Возвраты',          value: -k.returns_revenue, tint: 'var(--red)',    kind: 'expense', filter: 'returned' },
    { icon: '💸', label: 'Комиссия МП',       value: -k.commission,    tint: '#fb923c',         kind: 'expense', filter: 'commission' },
    { icon: '🚚', label: 'Логистика',         value: -k.logistics,     tint: '#60a5fa',         kind: 'expense', filter: 'logistics' },
    { icon: '🧾', label: 'Услуги/штрафы/реклама', value: -k.services,  tint: '#a78bfa',         kind: 'expense' },
    { icon: '📦', label: 'Себестоимость',     value: -k.cogs,          tint: '#22d3ee',         kind: 'expense' },
    { icon: '🏛',  label: 'Налог',             value: -k.tax,           tint: '#f472b6',         kind: 'expense' },
    { icon: '💰', label: 'Чистая прибыль',    value: k.net_profit,     tint: k.net_profit >= 0 ? 'var(--green)' : 'var(--red)', kind: 'total' },
  ];
      ...
      ${rows.map(r => {
        ...
        return `
          <div class="${cls}" style="--an-tint: ${r.tint}" ${onclick}>
            <div class="an2-pnl-icon">${r.icon}</div>
            <div>
