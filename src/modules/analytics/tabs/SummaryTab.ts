...
function renderSummaryTabUncached(k: KPI, orders: Order[], ts: TimeseriesPoint[], prevKpi: KPI | null, missingCogs: MissingCogsItem[] = []): string {
  ...
  return `
    ...
    <div class="an2-kpi-grid">
      ${renderKpiCard({ label: 'Выручка (выкуплено)', value: k.revenue, prev: prevKpi?.revenue, tint: 'rgba(212,240,0,.18)', sparkline: sparkRev, detail: 'revenue', hint: `заказано ${fmtMoney(k.ordered_revenue)}${k.in_transit_revenue > 0 ? ` · в пути ${fmtMoney(k.in_transit_revenue)}` : ''}`, tooltip: 'Сумма выкупленных заказов, по которым маркетплейс прислал финансовый отчёт. Заказы в пути и ожидающие расчёта в эту сумму не входят — они ещё могут быть отменены или возвращены.' })}
      ${renderKpiCard({ label: 'Выплата МП', value: k.revenue - k.commission - k.logistics - k.services - k.period_costs, prev: prevKpi != null ? prevKpi.revenue - prevKpi.commission - prevKpi.logistics - prevKpi.services - prevKpi.period_costs : null, tint: 'rgba(16,185,129,.18)', detail: 'gross', hint: `удержания МП ${fmtMoney(k.commission + k.logistics + k.services + k.period_costs)}`, tooltip: 'Сумма, которую маркетплейс переведёт вам на расчётный счёт: выручка минус комиссия, логистика, доп. услуги, реклама, хранение и штрафы. Себестоимость и налог здесь не учитываются — это ваши собственные расходы, не связанные с МП.' })}
      ${renderKpiCard({ label: 'Чистая прибыль', value: k.net_profit, prev: prevKpi?.net_profit, tint: 'rgba(34,197,94,.18)', sparkline: sparkProfit, detail: 'profit', hint: k.cogs > 0 ? `себестоимость ${fmtMoney(k.cogs)}` : 'себестоимость не заполнена', tooltip: 'Выплата МП минус себестоимость товара, налог и ваши ручные расходы (реклама, зарплата, аренда и т.д.). Это итог — сколько реально осталось после всех затрат.' })}
      ${renderKpiCard({ label: 'Маржа', value: k.margin_pct, prev: prevKpi?.margin_pct, format: 'pct', tint: 'rgba(96,165,250,.18)', detail: 'margin', hint: `${fmtMoney(k.orders_settled > 0 ? k.net_profit / k.orders_settled : 0)} с заказа`, tooltip: 'Чистая прибыль ÷ выручка × 100%. Показывает, какой процент выручки остаётся как прибыль после всех затрат. Норма для маркетплейсов — 5–20%.' })}
      ${renderKpiCard({ label: 'Заказов доставлено', value: k.orders_delivered, prev: prevKpi?.orders_delivered, format: 'int', tint: 'rgba(167,139,250,.18)', detail: 'delivered', hint: `выкуп ${k.buyout_pct.toFixed(0)}% · в работе ${fmtNum(k.orders_processing)}${k.awaiting_orders > 0 ? ` · ждут расчёта ${fmtNum(k.awaiting_orders)}` : ''}`, tooltip: 'Количество заказов, которые покупатель получил и не вернул. Только эти заказы участвуют в расчёте выручки и прибыли. Процент выкупа = доставлено ÷ (доставлено + возвращено + отменено).' })}
      ${renderKpiCard({ label: 'Расходы', value: k.total_expenses, prev: prevKpi?.total_expenses, tint: 'rgba(248,113,113,.18)', sparkline: sparkExp, invertDelta: true, detail: 'expenses', hint: `${k.revenue > 0 ? ((k.total_expenses / k.revenue) * 100).toFixed(0) : 0}% от выручки${k.period_costs > 0 ? ` · реклама и хранение ${fmtMoney(k.period_costs)}` : ''}`, tooltip: 'Все расходы периода: комиссия МП, логистика, услуги по заказам, реклама/хранение/штрафы, себестоимость товара, налог и ваши ручные расходы.' })}
      ${renderKpiCard({ label: 'Возвраты и отмены', value: k.orders_returned + k.orders_cancelled, prev: prevKpi != null ? prevKpi.orders_returned + prevKpi.orders_cancelled : null, format: 'int', tint: 'rgba(251,146,60,.18)', invertDelta: true, detail: 'returns', hint: `возвраты ${fmtNum(k.orders_returned)} · отмены ${fmtNum(k.orders_cancelled)}`, tooltip: 'Возвраты — покупатель получил товар, но вернул его обратно. Отмены — заказ отменён до доставки (покупателем или продавцом). Эти заказы не приносят выручки, но могут нести расходы по логистике возврата.' })}
    </div>
