...
1. Если ты не уверен что именно сделать — задай уточняющий вопрос ОБЫЧНЫМ ТЕКСТОМ, перечисли варианты.
2. Если понял — ответь СТРОГО одним JSON без лишних пояснений:
...
Пользователь увидит твой план и нажмёт «Выполнить» — только тогда действие выполнится.
