...
function wbRowToTransaction(row: WbFinanceRow, storeId: string): MpTransaction {
  ...
  return {
    ...
    items_json: row.nm_id ? [{ name: row.ts_name ?? row.sa_name ?? row.subject_name ?? '', sku: row.nm_id, quantity: row.quantity ?? 1 }] : null,
    accruals_for_sale: revenue,
    sale_commission: -Math.abs(commission),
    delivery_charge: -Math.abs(logistics),
    return_delivery_charge: 0,
    services_total: Math.abs(Number(row.storage_fee) || 0),
    amount: seller,
