...
import {
  ...
  getWidgetDims, setWidgetDims, clampDims,
} from './modules/HomeWidgets';
...
export class App {
  ...
  private esc(s: unknown): string { return escHtml(s); }
  ...
  private static readonly SKIP_COLS = new Set([
    'Ссылка на главное фото*', 'Ссылки на дополнительные фото', 'Ссылки на фото 360',
  ...
  private renderCards(content: HTMLElement) {
   ...
   */
  private async pushProductToOzon(
    _productId: string,
...
async doDeleteProductFromBox() {
  ...
  private getDashboardStats() {
    const allProds = boxes.get().flatMap(b => this.cache.get(b.id) || []);
    const total = allProds.length;

    // Box stats
    const boxStats = boxes.get().map(b => {
      const prods = this.cache.get(b.id) || [];
      return { id: b.id, name: b.name, sticker: b.sticker || '📦', count: prods.length };
    });

    // Price stats
    const prices = allProds
      .map(p => parseFloat(p.data?.['Цена, руб.*'] || '0'))
      .filter(n => n > 0);
    ...
    const buckets = [
      { label: '<5к', min: 0, max: 5000, count: 0 },
      { label: '5-10к', min: 5000, max: 10000, count: 0 },
      { label: '10-20к', min: 10000, max: 20000, count: 0 },
      { label: '20-50к', min: 20000, max: 50000, count: 0 },
      { label: '50к+', min: 50000, max: Infinity, count: 0 },
    ];
    prices.forEach(p => {
    ...
    // Recent products (last 8 by array order — most recently added are at front since we unshift)
    const recentProducts = allProds.slice(0, 8);

    return { total, boxStats, avgPrice, maxPrice, minPrice, topCategories, topBrands, buckets, recentProducts };
  }
  ...
  /** Просто обёртка чтобы старый код с getWidgetDims продолжал работать */
  _useWidgetDims() { return { getWidgetDims, setWidgetDims }; }
  ...
  private async populateLiveCommandCenter(): Promise<void> {
        ...
        grid.innerHTML = storeCards.map(c => {
          ...
          return `
            <div class="sf-row" style="--c:${c.mpColor}" onclick="window.app.navigateTo('${dest}')"
              title="${this.esc(c.storeName)} · ${lastAgo}">
              <span class="sf-dot"></span>
              <span class="sf-name">${this.esc(c.storeName)}</span>
        ...
        : recent.map(o => `
          <div class="cmd-feed-item" onclick="window.app.navigateTo('orders')">
            ...
            <div class="cmd-feed-info">
              <div class="cmd-feed-name">${this.esc(o.productName || o.id)}</div>
        ...
        : `
          ...
          ${urgent.map(o => `
            <div class="cmd-action-item" onclick="window.app.navigateTo('orders-${o.mp === 'ozon' ? 'ozon' : o.mp === 'yandex' ? 'yandex' : 'wb'}')">
              <span class="cmd-feed-mp cmd-feed-mp-${o.mp}">${o.mp === 'ozon' ? 'OZ' : o.mp === 'yandex' ? 'ЯМ' : 'WB'}</span>
              <div style="flex:1;min-width:0">
                <div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${this.esc(o.productName || o.id)}</div>
                <div style="font-size:10px;color:var(--text3);margin-top:2px">${this.esc(o.storeName)} · создан ${this.timeAgo(o.created)}</div>
              </div>
              ...
              return `
                <div style="margin-bottom:10px">
                  <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px">
                    <span style="font-weight:600">${this.esc(v.name)} · ${v.orders} зак.</span>
        ...
        : `<div style="padding:8px 14px 14px">
            ${top.map((p, i) => `
              <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
                ...
                <div style="flex:1;min-width:0">
                  <div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${this.esc(p.name)}</div>
    ...
    setTimeout(() => {
      ...
      if (el && this.currentPage === 'home') el.textContent = '10 сек назад';
    }, 10000);
  }
  ...
  /** @deprecated Legacy. Не используется в новой Главной. */
  // @ts-expect-error
  private renderDashWidget(id: string, contentHtml: string): void {
    const el = document.getElementById(id);
    if (!el) return;
    const titleEl = el.querySelector('.dash-widget-title');
    el.innerHTML = (titleEl?.outerHTML ?? '') + contentHtml;
  }
  ...
  async viewProductFromDash(productId: string, boxId: string) {
    ...
    if (!this.cache.has(boxId)) {
      await this.loadBoxProducts();
    } else {
      this.allProducts = this.cache.get(boxId)!;
      this.buildColumns();
      this.applyFilters();
      this.updateResultStat();
      const exportBtn = document.getElementById('export-btn');
      if (exportBtn && this.allProducts.length) exportBtn.style.display = 'flex';
    }
  ...
  massDeleteProducts() {
    ...
    this.openModal('Удалить выбранные товары?', `Будет удалено товаров: ${this.selectedProducts.size}`,
  ...
  async doMassDelete() {
    ...
    try {
      ...
      this.cache.delete(this.activeBoxId!);
      await this.loadBoxProducts();
      this.selectedProducts.clear();
      this.renderActionBar();
      this.closeModal();
      this.toast(`Удалено товаров: ${ids.length}`, 'success');
    } catch (e: any) { this.toast('Ошибка удаления: ' + e.message, 'error'); }
  }
  ...
  massHideProducts(hide: boolean) {
    ...
    selectedProds.forEach(p => {
      let set = this.hiddenRows.get(p.box_id);
      if (!set) {
        set = new Set();
        this.hiddenRows.set(p.box_id, set);
      }
    ...
    const obj: any = {};
    this.hiddenRows.forEach((set, boxId) => {
    ...
    this.renderProducts();
    this.renderActionBar();
    this.toast(hide ? 'Товары скрыты' : 'Товары показаны', 'success');
  }

  massMoveProducts() {
    if (this.selectedProducts.size === 0) return;
    const allBoxes = boxes.get().filter(b => b.id !== this.activeBoxId);
    if (allBoxes.length === 0) {
      this.toast('Нет других групп для перемещения', 'error');
      return;
    }
    const opts = allBoxes.map(b => `<option value="${b.id}">${this.esc(b.sticker || '')} ${this.esc(b.name)}</option>`).join('');
    
    this.openModal('Переместить товары', `Выберите группу для ${this.selectedProducts.size} товаров:`,
  ...
  async doMassMove() {
    ...
    try {
      ...
      this.cache.delete(this.activeBoxId!);
      this.cache.delete(targetBoxId);
      await this.loadBoxProducts();
      this.selectedProducts.clear();
      this.renderActionBar();
      this.closeModal();
      this.toast(`Перемещено товаров: ${ids.length}`, 'success');
    } catch (e: any) { this.toast('Ошибка перемещения: ' + e.message, 'error'); }
  }

  openMassEditModal() {
    if (this.selectedProducts.size === 0) return;
    const prods = this.allProducts.filter(p => this.selectedProducts.has(p.id));
    
    // Показываем видимые сейчас столбцы
    const allCols = this.columns.filter(c => !App.SKIP_COLS.has(c));
    const editCols = this.visibleCols ? allCols.filter(c => this.visibleCols!.has(c)) : allCols;
    
    let rowsHtml = '';
    for (const p of prods) {
      rowsHtml += `<tr data-id="${p.id}" class="mass-edit-row">`;
      for (const col of editCols) {
        const val = this.esc(String(p.data[col] || ''));
        const readonly = col.includes('Артикул'); // артикул менять нельзя, он ключ
        rowsHtml += `
          <td style="padding:4px;border:1px solid var(--border)">
            <input type="text" class="form-input" data-col="${this.esc(col)}" value="${val}" ${readonly ? 'disabled style="background:var(--bg2);opacity:0.7;padding:4px 6px;height:28px;font-size:12px;border:none"' : 'style="padding:4px 6px;height:28px;font-size:12px;border:none;background:transparent"'} />
          </td>
        `;
      }
      rowsHtml += `</tr>`;
      ...
      <div style="overflow-x:auto;max-height:60vh;overflow-y:auto;border:1px solid var(--border);border-radius:6px;background:var(--bg2)">
        <table style="width:100%;border-collapse:collapse;min-width:600px">
          <thead style="position:sticky;top:0;z-index:2;background:var(--bg3)">
            <tr>
              ${editCols.map(c => `<th style="padding:6px;text-align:left;font-size:11px;color:var(--text3);border:1px solid var(--border);white-space:nowrap">${this.esc(c.replace('*', ''))}</th>`).join('')}
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
    ...
    this.openModalLg('Массовое редактирование', `${prods.length} товаров`,
      tableHtml,
      `<button class="btn" onclick="window.app.closeModal()">Отмена</button>
  ...
  async doMassEditSave() {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (changed) {
          ...
          if (isOzonLinked && newData['_ozon_synced_at']) {
            await this.pushProductToOzon(id, newData, box!);
          }
        }
      }
      ...
      if (updatedCount > 0) {
        this.cache.delete(this.activeBoxId!);
        if (this.viewMode === 'table') this.vtPaint();
        else this.renderCards(document.getElementById('list-content')!);
        this.toast(`Обновлено товаров: ${updatedCount}`, 'success');
      } else {
        this.toast('Нет изменений', 'info');
      }
      this.closeModal();
    } catch (e: any) {
      this.toast('Ошибка сохранения: ' + e.message, 'error');
  ...
  async openMpSyncModal() {
    ...
    if (!cid) { this.toast('Нет активной компании', 'error'); return; }
    ...
    const renderRow = (icon: string, label: string, source: string, s: any) => {
      ...
      return `
        <div class="mp-sync-row" style="display:flex;align-items:center;gap:12px;padding:12px;border:1px solid var(--border);border-radius:10px;margin-bottom:8px">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-weight:600;font-size:13px">${label}: ${this.esc(s.name)}</div>
  ...
  async runMpSync(source: 'ozon' | 'ym' | 'wb', storeId: string, btn: HTMLButtonElement) {
    ...
    try {
      ...
      this.toast(`✅ ${result.boxName}: ${result.created} добавлено, ${result.updated} обновлено`, 'success', 5000);

      // Переходим в синхронизированную группу
      if (result.boxId) {
        setTimeout(async () => {
          this.closeModal();
          await this.selectBox(result.boxId);
        }, 1500);
      }
    } catch (e: any) {
      ...
      this.toast('Ошибка синхронизации: ' + e.message, 'error', 6000);
  ...
  openMassFillModal() {
    ...
    const target = this.selectedProducts.size > 0
      ? `выбранных ${this.selectedProducts.size} товаров`
      : `всех ${this.filtered.length} товаров`;

    this.openModalLg('Заполнить колонку',
      ...
      `<div style="display:flex;flex-direction:column;gap:12px">
        <div>
          ...
          <select id="mf-col" class="form-select" style="width:100%">
            ${cols.map(c => `<option value="${this.esc(c)}">${this.esc(c.replace('*',''))}</option>`).join('')}
          </select>
        </div>
        <div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
            <input type="radio" name="mf-mode" value="all" checked> Применить ко всем отфильтрованным (${this.filtered.length})
          </label>
          ${this.selectedProducts.size > 0 ? `
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
            <input type="radio" name="mf-mode" value="selected"> Только выбранным (${this.selectedProducts.size})
          </label>` : ''}
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer">
  ...
  async applyMassFill() {
    ...
    if (!col) { this.toast('Выберите колонку', 'error'); return; }

    let targets = this.filtered;
    if (mode === 'selected' && this.selectedProducts.size > 0) {
      targets = this.filtered.filter(p => this.selectedProducts.has(p.id));
    } else if (mode === 'empty') {
      targets = this.filtered.filter(p => {
        const v = (p.data || {})[col];
        return v === undefined || v === null || v === '';
      });
    }

    if (!targets.length) { this.toast('Нет подходящих товаров', 'info'); return; }
    ...
    this.toast(`✅ Заполнено ${done} товаров: «${col.replace('*','')}» = «${value}»`, 'success', 5000);
    this.closeModal();
    this.buildColumns();
    this.applyFilters();
    if (this.viewMode === 'table') this.vtPaint();
